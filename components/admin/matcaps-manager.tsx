...
export function MatcapsManager() {
  const { matcaps, updateMatcap, deleteMatcap, addMatcap } = useResources()
  
  const [showForm, setShowForm] = useState(false)
  const [newMatcap, setNewMatcap] = useState({
    name: '',
    matcap: '',
  })

  const matcapsList = Object.values(matcaps)

  const handleAddMatcap = () => {
    if (newMatcap.name.trim() && newMatcap.matcap.trim()) {
      const id = `matcap_${Date.now()}`
      addMatcap({
        id,
        name: newMatcap.name,
        matcap: newMatcap.matcap,
        active: true,
      })
      setNewMatcap({ name: '', matcap: '' })
      setShowForm(false)
    }
  }

  const handleToggleActive = (id: string) => {
    const matcap = matcaps[id]
    if (matcap) {
      updateMatcap(id, { active: !matcap.active })
    }
  }
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        setNewMatcap({
          ...newMatcap,
          matcap: event.target?.result as string,
          name: newMatcap.name || file.name.replace(/\.[^/.]+$/, ""),
        })
      }
        ...
        <Card className="bg-[#2a2a2a] border-[#404040]">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-2 pt-4">
              <Button
                onClick={() => { setShowForm(false); setNewMatcap({ name: '', matcap: '' }); }}
                variant="outline"
        ...
        {matcapsList.map((matcap) => (
          <Card key={matcap.id} className={`bg-[#2a2a2a] border-[#404040] overflow-hidden group ${!matcap.active ? 'opacity-50' : ''}`}>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent className="bg-[#2a2a2a] border-[#404040]">
                    ...
                    <div className="flex gap-2">
                      ...
                      <AlertDialogAction
                        onClick={() => deleteMatcap(matcap.id)}
            ...
            <CardContent className="p-2">
              ...
              <p className={`text-[10px] text-center ${matcap.active ? 'text-cyan-400' : 'text-zinc-500'}`}>
                {matcap.active ? 'Active' : 'Inactive'}
              </p>
            </CardContent>
          </Card>
        ))}
